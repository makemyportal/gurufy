      ...
      <div className="relative overflow-hidden rounded-2xl mb-6 sm:mb-8 mt-2 bg-white/40 dark:bg-[#111118]/60 backdrop-blur-3xl min-h-[140px] sm:min-h-[180px] flex items-center px-4 sm:px-10 border border-white/60 dark:border-white/10 shadow-sm">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between w-full min-w-0 gap-6 py-8 lg:py-0">
          ...
          <div className="max-w-xl min-w-0 pl-2">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium tracking-wide max-w-[420px] leading-relaxed">
            ...
            <div className="flex flex-wrap items-center gap-3 md:gap-4 w-full md:w-auto mt-2 lg:mt-0 lg:ml-auto">
              ...
              <div className="flex items-center gap-3 bg-white/70 dark:bg-white/5 backdrop-blur-md border border-white/80 dark:border-white/10 rounded-2xl py-2.5 px-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group cursor-pointer hidden md:flex">
                ...
                <div className="flex flex-col pr-2">
                  ...
                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">XP</span>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5 lg:gap-8">
        ...
        <div className="space-y-4 sm:space-y-5 min-w-0 w-full">
          ...
          {currentUser ? (
            ...
            <form onSubmit={handleCreatePost}>
                <div className="flex gap-4 p-5 pb-2">
                  ...
                  <textarea
                    ...
                    placeholder={postType === 'poll' ? "Ask a question for your poll..." : postType === 'question' ? "Ask your question to the community... ❓" : postType === 'announcement' ? "Write your announcement..." : "Share a resource, ask a question, or start a discussion..."}
                    rows={composerFocused || newPost ? 3 : 1}
                    className="flex-1 resize-none text-base text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none leading-relaxed pt-2 transition-all duration-300 bg-transparent font-medium"
                  />
                </div>
                ...
                {postType === 'poll' && (
                  <div className="mx-5 mt-3 pl-12 pr-4 space-y-2.5 animate-fade-in-up">
                    ...
                    {pollOptions.length < 4 && (
                      <button type="button" onClick={() => setPollOptions(p => [...p, { id: Date.now(), text: '' }])} className="text-[13px] font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 flex items-center gap-1.5 pt-2 ml-5 transition-colors">
                ...
                {postType === 'youtube' && (
                  <div className="mx-5 mt-3 animate-fade-in-up">
                    <input type="url" placeholder="Paste YouTube Link (e.g. https://youtube.com/watch?v=...)" value={youtubeLink} onChange={e => setYoutubeLink(e.target.value)}
                      className="w-full bg-slate-50 dark:bg-black/20 border border-slate-200/60 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:bg-white dark:focus:bg-[#1a1a24] focus:border-red-400 focus:ring-4 focus:ring-red-100 dark:focus:ring-red-900/20 outline-none transition-all placeholder:text-slate-400 text-slate-800 dark:text-slate-200" />
                  </div>
                )}
                ...
                {postType === 'link' && (
                  <div className="mx-5 mt-3 animate-fade-in-up">
                    <input type="url" placeholder="Paste any URL (e.g. https://example.com/article)" value={linkUrl} onChange={e => setLinkUrl(e.target.value)}
                      className="w-full bg-slate-50 dark:bg-black/20 border border-slate-200/60 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:bg-white dark:focus:bg-[#1a1a24] focus:border-blue-400 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 outline-none transition-all placeholder:text-slate-400 text-slate-800 dark:text-slate-200" />
                  </div>
                )}

                {filePreview && (
                  <div className="mx-5 mt-3 bg-slate-50 dark:bg-white/5 rounded-2xl p-3 flex items-center gap-3 border border-slate-200/60 dark:border-white/10 relative overflow-hidden group">
                    {filePreview.type === 'image'
                      ? <img src={filePreview.url} alt="" className="w-14 h-14 rounded-xl object-cover shadow-sm" />
                      : <div className="w-14 h-14 bg-indigo-100 dark:bg-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-700 dark:text-indigo-400 font-bold text-xs">{filePreview.ext}</div>}
                    <p className="text-sm text-slate-700 dark:text-slate-300 font-bold flex-1 truncate pr-8">{filePreview.name}</p>
                    <button type="button" onClick={clearFile} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-white/80 dark:bg-black/40 backdrop-blur-sm shadow-sm hover:bg-rose-50 dark:hover:bg-rose-500/20 rounded-xl opacity-0 group-hover:opacity-100 transition-all"><X className="w-4 h-4 text-slate-500 hover:text-rose-600" /></button>
                  </div>
                )}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-5 py-3 mt-4 border-t border-slate-100 dark:border-white/5 relative">
                  <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
                    ...
                    <label htmlFor="feed-photo" className="flex items-center gap-1.5 px-3 py-2 text-[13px] font-bold text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-white/5 rounded-xl cursor-pointer transition-all">
                    ...
                    <label htmlFor="feed-file" className="flex items-center gap-1.5 px-3 py-2 text-[13px] font-bold text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-white/5 rounded-xl cursor-pointer transition-all">
                      <ImagePlus className="w-4 h-4" /> <span className="hidden sm:inline">File</span>
                    </label>
                    ...
                    {['admin', 'superadmin'].includes(userProfile?.role) && (
                      <div className="flex items-center border-l-2 border-slate-100 dark:border-white/5 pl-2 ml-1">
                        ...
                        <button type="button" onClick={handleGenerateAIPost} disabled={generatingAI}
                          className="flex items-center gap-1.5 px-3 py-2 text-[13px] font-black text-fuchsia-600 dark:text-fuchsia-400 hover:bg-fuchsia-50 dark:hover:bg-white/5 rounded-xl transition-all disabled:opacity-50">
                          {generatingAI ? <Loader2 className="w-4 h-4 animate-spin" /> : <Bot className="w-4 h-4" />}
                          <span className="hidden xl:inline">AI Post</span>
                        </button>
                      </div>
                    )}
                  </div>
                  <button type="submit" disabled={posting || (!newPost.trim() && !selectedFile && postType === 'text' && !youtubeLink.trim() && !linkUrl.trim())}
                    className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:shadow-none text-white text-sm font-extrabold rounded-xl transition-all hover:shadow-lg hover:shadow-indigo-500/30 active:scale-[0.97] shrink-0">
                    {posting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                    <span className="hidden sm:inline">{posting ? 'Publishing...' : t('publish', 'Post')}</span>
                  </button>
                </div>
              </form>
            </div>
            ...
            /* Guest CTA */
            <div className="relative overflow-hidden bg-gradient-to-br from-indigo-50 to-white dark:from-indigo-900/10 dark:to-[#1a1a24] border border-indigo-100 dark:border-white/5 rounded-[24px] p-8 text-center shadow-sm">
              <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-400/10 rounded-full -mr-32 -mt-32 blur-3xl pointer-events-none" />
              <div className="relative z-10">
                <div className="w-14 h-14 rounded-2xl bg-indigo-100 dark:bg-indigo-500/20 flex items-center justify-center mx-auto mb-4 shadow-inner">
                  <PenLine className="w-7 h-7 text-indigo-600 dark:text-indigo-400" />
                </div>
                <h3 className="font-black text-slate-800 dark:text-white text-xl mb-2">{t('joinNetwork', 'Join the Community')}</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-6 max-w-sm mx-auto leading-relaxed">
                  {t('joinDesc', 'Sign in to share your ideas, ask questions, and interact with educators across India.')}
                </p>
                <button onClick={() => navigate('/login')}
                  className="inline-flex items-center gap-2 px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-sm rounded-xl transition-all shadow-[0_8px_30px_rgb(99,102,241,0.2)] hover:shadow-[0_8px_30px_rgb(99,102,241,0.4)] hover:-translate-y-0.5 active:scale-95">
                  {t('signInContribute', 'Sign In to Post')} <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between px-4 sm:px-1 py-1">
            <div className="flex items-center gap-2">
        ...
        <div className="hidden lg:block space-y-5">
          <div className="sticky top-[90px] space-y-5">
            ...
            {globalAnnouncement && (
              <div className="bg-gradient-to-br from-indigo-600 via-violet-600 to-primary-600 rounded-[24px] p-6 text-white shadow-[0_8px_30px_rgba(99,102,241,0.3)] relative overflow-hidden animate-fade-in-up">
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-10 -mt-10 blur-2xl" />
                <div className="absolute bottom-0 left-0 w-24 h-24 bg-pink-500/20 rounded-full -ml-10 -mb-10 blur-xl" />
                <div className="flex items-start gap-4 relative z-10">
                  ...
                  <div>
                    <h3 className="font-black text-[11px] tracking-[0.2em] mb-2 flex items-center gap-2 uppercase text-indigo-100">
                      Admin Broadcast
            ...
            <div className="bg-white/80 dark:bg-[#1a1a24]/80 backdrop-blur-xl border border-slate-200/60 dark:border-white/5 rounded-[24px] overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-slate-100 dark:border-white/5">
                <div className="flex items-center gap-2.5">
                  ...
                  <p className="text-[12px] font-black text-slate-800 dark:text-white uppercase tracking-widest">Trending Now</p>
                </div>
                <span className="flex h-2 w-2 relative">
              ...
              <div className="divide-y divide-slate-100/60 dark:divide-white/5">
                {sidebarTrending.map((item, idx) => (
                  <div key={`sidebar-${idx}`} onClick={() => item.url && window.open(item.url, '_blank', 'noopener,noreferrer')} className="px-5 py-4 hover:bg-indigo-50/50 dark:hover:bg-white/5 transition-colors cursor-pointer group">
                    <div className="flex items-start gap-3">
                      <span className="text-xl mt-0.5 shrink-0 group-hover:scale-110 transition-transform">{item.icon}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-800 dark:text-slate-200 leading-tight line-clamp-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                          {item.title}
                        </p>
                        <div className="flex items-center justify-between mt-2.5">
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-extrabold uppercase tracking-wider bg-gradient-to-r ${item.tagColor} bg-clip-text text-transparent`}>
                              {item.tag}
                            </span>
                            <span className="text-[10px] text-slate-300 dark:text-slate-600">·</span>
                            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{item.time}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="px-5 py-3 border-t border-slate-100/60 dark:border-white/5 bg-slate-50/50 dark:bg-black/10">
                <button className="w-full text-center text-[11px] font-extrabold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors uppercase tracking-wider">
                  View All Updates →
                </button>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-emerald-50/80 to-green-50/80 dark:from-emerald-900/20 dark:to-green-900/20 border border-emerald-200/60 dark:border-emerald-800/40 rounded-[24px] p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
              ...
              <div className="flex items-center gap-2.5 mb-5 relative z-10">
                ...
                <p className="text-[12px] font-black text-emerald-900 dark:text-emerald-300 uppercase tracking-widest">Latest Naukri</p>
              </div>
              <div className="space-y-3 relative z-10">
                {getItemsByCategory('job').slice(0, 3).map((job, idx) => (
                  <div key={`sidebar-job-${idx}`} onClick={() => job.url && window.open(job.url, '_blank', 'noopener,noreferrer')} className="bg-white/80 dark:bg-black/20 backdrop-blur-sm rounded-xl p-3.5 border border-emerald-100/50 dark:border-white/5 hover:border-emerald-300 dark:hover:border-emerald-500/40 hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer group">
                    <p className="text-[13px] font-extrabold text-slate-800 dark:text-slate-200 leading-tight mb-1.5 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors line-clamp-1">
                      {job.icon} {job.title}
                    </p>
                    {job.salary && <p className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 mb-2">{job.salary}</p>}
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/30 rounded-md">{job.location}</span>
                      <span className="text-[10px] font-bold text-rose-500 flex items-center gap-1"><Heart className="w-3 h-3 fill-rose-500" /> {job.likes}</span>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => navigate('/jobs')} className="w-full mt-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-[13px] rounded-xl transition-all flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-emerald-500/30 active:scale-[0.98] relative z-10">
                <Briefcase className="w-4 h-4" /> View All Jobs
              </button>
            </div>

            {/* Stats Card */}
            <div className="bg-white/80 dark:bg-[#1a1a24]/80 backdrop-blur-xl border border-slate-200/60 dark:border-white/5 rounded-[24px] p-6 shadow-sm">
              <p className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-5">Platform Activity</p>
              <div className="space-y-4">
                {[
                  { icon: FileText, label: 'Knowledge Posts', val: posts.length, color: 'text-indigo-500 bg-indigo-50 border-indigo-100 dark:bg-indigo-500/10 dark:border-indigo-500/20' },
                  { icon: Heart, label: 'Total Likes', val: totalLikes, color: 'text-rose-500 bg-rose-50 border-rose-100 dark:bg-rose-500/10 dark:border-rose-500/20' },
                  { icon: Users, label: 'Contributors', val: [...new Set(posts.map(p => p.authorId))].length, color: 'text-emerald-500 bg-emerald-50 border-emerald-100 dark:bg-emerald-500/10 dark:border-emerald-500/20' },
                  { icon: Newspaper, label: 'Daily Updates', val: ALL_AUTO_FEED.length, color: 'text-blue-500 bg-blue-50 border-blue-100 dark:bg-blue-500/10 dark:border-blue-500/20' },
                ].map(({ icon: Icon, label, val, color }) => (
                  <div key={label} className="flex items-center justify-between group cursor-default">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center border ${color} group-hover:scale-110 transition-transform`}>
            ...
            {/* Explote CTA Premium */}
            <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-[#0f0f14] dark:from-black dark:to-[#0f0f14] border border-slate-800 dark:border-white/10 rounded-[24px] p-6 shadow-xl">
              <div className="absolute top-[-30%] right-[-20%] w-32 h-32 bg-indigo-500/40 rounded-full blur-[40px] pointer-events-none" />
              <div className="absolute bottom-[-10%] left-[-10%] w-24 h-24 bg-fuchsia-500/30 rounded-full blur-[30px] pointer-events-none" />
              <div className="relative z-10">
                <div className="w-10 h-10 rounded-xl bg-indigo-500/20 border border-indigo-400/30 flex items-center justify-center mb-4 backdrop-blur-md">
                  <Zap className="w-5 h-5 text-indigo-300" />
                </div>
                <p className="text-white font-black text-base mb-1.5 tracking-wide">Supercharge Teaching</p>
                <p className="text-slate-400 text-[13px] font-medium mb-6 leading-relaxed">
                  Use our top-tier AI tools to generate lesson plans, worksheets & quizzes instantly.
                </p>
                <button onClick={() => navigate('/ai-tools')}
                  className="w-full py-3 bg-indigo-500 hover:bg-indigo-400 text-white font-extrabold text-[13px] rounded-xl transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(99,102,241,0.3)] hover:shadow-[0_0_30px_rgba(99,102,241,0.5)] active:scale-[0.98]">
                  <Sparkles className="w-4 h-4" /> Launch AI Magic
                </button>
              </div>
            </div>
