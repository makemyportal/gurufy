...
export function GamificationProvider({ children }) {
  ...
  useEffect(() => {
    if (!currentUser) {
      setStats({ xp: 0, streak: 0, longestStreak: 0, lastLoginDate: null, badges: [], totalPosts: 0, totalLikes: 0, totalComments: 0, totalResources: 0, aiUsages: 0 })
      setLoading(false)
      return
    }
  ...
  async function awardXP(amount, reason, badgeCheck) {
    ...
    try {
      const ref = doc(db, 'gamification', currentUser.uid)
      const updates = { xp: increment(amount) }
